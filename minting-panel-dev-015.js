...
(function() {
    ...
    const SnobMinter = {
        ...
        updateNobleInfoUI: function() {
            ...
            container.innerHTML = `
                <div style="display: flex; gap: 20px; flex-wrap: wrap; padding: 8px 12px; background: #f5f0e0; border-radius: 6px; border: 1px solid #e0d5c0;">
        ...
        parseVillagesWithSnob: function(html) {
            ...
            if (table) {
                console.log('[SnobMinter] Классы таблицы:', table.className);
            }
            ...
            if (table) {
                ...
                if (innerContainers.length > 0) {
                    ...
                    for (const container of innerContainers) {
                        ...
                        if (/^\d+$/.test(containerId)) {
                            ...
                            console.log('[SnobMinter] Пропущен контейнер (ID не является числом):', containerId);
                        }
                    }
                } else {
                    ...
                    for (const tr of trs) {
                        console.log('[SnobMinter] Строка ID:', tr.id);
                        rows.push(tr);
                    }
                }
            } else {
                ...
                for (const container of containers) {
                    ...
                    if (containerId && /^\d+$/.test(containerId)) {
                        console.log('[SnobMinter] Добавлен контейнер деревни (альт):', containerId);
                        rows.push({
                            container: container,
                            id: containerId,
                            isContainer: true
            ...
            console.log('[SnobMinter] Всего найдено записей для обработки:', rows.length);
            
            for (const row of rows) {
                let villageId, name = '', coord = '', level = 0, warehouse = 0, queueCount = 0;
                
                console.log('[SnobMinter] --- Обработка записи ---');
                
                if (row.isContainer) {
                    // Мобильная версия
                    console.log('[SnobMinter] Обработка мобильного контейнера');
                    ...
                    if (quickeditSpan) {
                        ...
                        console.log('[SnobMinter] .quickedit-label найден:', !!labelSpan);
                        
                        if (labelSpan) {
                            const fullText = labelSpan.textContent.trim();
                            console.log('[SnobMinter] Текст label:', fullText);
                            
                            const coordMatch = fullText.match(/\((\d{1,3}\|\d{1,3})\)/);
                            if (coordMatch) {
                                coord = coordMatch[1];
                                name = fullText.replace(/\(\d{1,3}\|\d{1,3}\)/, '').trim();
                                console.log('[SnobMinter] Название:', name, 'Координаты:', coord);
                            } else {
                                name = fullText;
                                coord = '?|?';
                                console.log('[SnobMinter] Координаты не найдены, имя:', name);
                            }
                    ...
                    // ПК версия
                    console.log('[SnobMinter] Обработка ПК строки');
                    const tr = row;
                ...
                console.log('[SnobMinter] ✅ Деревня ДОБАВЛЕНА:', name, '(', coord, ') ID:', villageId);
        ...
        stopSingleMinting: async function(villageId) {
            ...
            if (!village.isMinting) {
                UI.ErrorMessage('Чеканка уже не активна!');
                return;
            }
        ...
        updateUI: function() {
            // Обновляем чекбоксы в карточках
            for (const v of this.villages) {
                const checkbox = document.querySelector(`#snob-cards-container input[data-id="${v.id}"]:not(.snob-auto-start)`);
                if (checkbox) {
        ...
        createGUI: function() {
            ...
            styleEl.textContent = `
                #tw-snob-minter * { box-sizing: border-box; }
                #tw-snob-minter .btn {
                ...
                #tw-snob-minter .village-card .card-checkbox { flex: 0 0 24px; }
                ...
                #tw-snob-minter .village-card .card-auto { flex: 0 0 30px; text-align: center; }
                #tw-snob-minter .village-card .card-action { flex: 0 0 35px; text-align: center; }
                ...
                #tw-snob-minter .village-card .card-action .snob-action-btn.btn-confirm-yes { background: #28a745; }
                #tw-snob-minter .village-card .card-action .snob-action-btn.btn-confirm-no { background: #dc3545; }
                ...
                @media (max-width: 480px) {
                    #tw-snob-minter { padding: 4px; margin: 4px auto; }
                    #tw-snob-minter .village-card { padding: 6px; gap: 4px; }
                    #tw-snob-minter .village-card .card-name { font-size: 10px; flex: 1 1 80px; }
                    #tw-snob-minter .village-card .card-status { font-size: 8px; flex: 0 0 60px; }
                    #tw-snob-minter .village-card .card-coord { font-size: 8px; flex: 0 0 55px; }
                    #tw-snob-minter .village-card .card-completion { font-size: 7px; }
                    #tw-snob-minter .controls-row .btn { font-size: 8px; padding: 2px 6px; min-width: 50px; }
                    #tw-snob-minter .noble-info { font-size: 8px; padding: 4px 6px; }
                    #tw-snob-minter .noble-info span { flex: 1 1 100%; }
                }
            `;
            ...
            div.innerHTML = `
                ...
                <div class="help-panel" id="snob-help-panel">
                    ...
                    <div class="help-content">${this.t('helpText')}</div>
                </div>
                ...
                <div class="controls-row">
                    <button id="snob-refresh-btn" class="btn">🔄 ${this.t('refresh')}</button>
        ...
        refreshVillages: async function() {
            ...
            container.innerHTML = `<div style="text-align: center; padding: 20px; color: #6c757d;">${this.t('loading')}</div>`;
            ...
            try {
                ...
                container.innerHTML = `<div style="text-align: center; padding: 20px; color: #dc3545;">❌ Ошибка загрузки: ${error.message}</div>`;
